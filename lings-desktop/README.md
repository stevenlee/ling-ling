...
> 📐 想了解系統**怎麼運作**（架構、資料流、設計取捨）？見 [`System_Engine/DesignDoc/SystemDesign.md`](../System_Engine/DesignDoc/SystemDesign.md)。這份 README 只講「**怎麼用**」。
...
- [使用方式](#使用方式)
- [指令一覽](#指令一覽)
- [TUI cockpit（終端機操作台）](#tui-cockpit終端機操作台)
- [專案結構](#專案結構)
- [架構演進](#架構演進)
- [開發檢查](#開發檢查)
- [License](#license)
...
| `OLLAMA_API_BASE` / `OLLAMA_MODEL` | 本地 Ollama 端點與模型 |
| `VLLM_API_BASE` / `VLLM_MODEL` | 遠端 vLLM 端點與模型 |
| `GEMINI_API_KEY` / `GEMINI_MODEL` | Gemini API 金鑰與模型 |
| `CHUNK_SIZE` / `CHUNK_OVERLAP` | 切分大小與重疊 |
...
> 想知道這些模組彼此怎麼呼叫、資料怎麼流動，見 [`SystemDesign.md`](../System_Engine/DesignDoc/SystemDesign.md)。

## 架構演進

逐項變更紀錄見 [CHANGELOG.md](CHANGELOG.md)；架構細節見 [`SystemDesign.md`](../System_Engine/DesignDoc/SystemDesign.md)。主要里程碑（新到舊）：
...
Markdown/Mermaid 迴歸測試：

```bash
PYTHONPATH="$PWD/System_Engine" venv/bin/python -m pytest -q System_Engine/tests/test_mermaid.py
```

LingLens (CounterAgent) 迴歸測試：

```bash
PYTHONPATH="$PWD/System_Engine" venv/bin/python -m pytest -q System_Engine/tests/test_counter_agent.py
```
