...
CORTEX_CONSOLIDATION_QUARANTINE_HOURS default 24 # half-open outage recovery
...
2. **抽取主張**：每份 insight 一次 `extract_claims` call（配額
   ...
   `CORTEX_CONSOLIDATION_MAX_ATTEMPTS` 後 quarantine；TTL 到期 half-open
   探測一次，讓 outage 恢復後自癒。內容改變即重設 retry budget。
