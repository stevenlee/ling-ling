...
| R6 | **不做（2026-06-13 結案）。** ROI 不成立:bench 是監控指標、非使用者回報的問題;73% 拆開只有 2-3 條真 miss(其餘是無害跨版本/同領域),且只影響 `@ling` 問答檢索一條路徑;修法(DGX 立 TEI 服務 + 可能全 vault 重建索引)是為「不確定有沒有在痛的功能」投真實基建。等真的被爛答案咬到再重啟。原診斷保留如下↓。 ~~**檢索品質漂移**：bench 100%（5月底）→ 73%（6/11+）。**已診斷（2026-06-13），非 code 回歸**：純 hybrid（vector+BM25）檢索隨索引增長碰撞加劇，cross-encoder reranker 未裝也未開（`RERANKER_ENABLED=false`、`sentence-transformers` 未安裝）。失敗模式：同一母文件 chunk 洗版（Hardy Synthesis 6 個 chunk 佔 rank 2–8），把預期文件擠到 rank 9–10。**per-doc cap 不可行**——會打爛合法依賴同文件多 chunk 的查詢（Lax-Milgram 的 top-5 正是同一本書 5 個 chunk）。Trench 對 Taylor 查詢甚至不在 top-30（embedder recall 缺口，中文查詢更明顯：弱解→Hamlet）。**真正的修法是基建決策**：裝並開 cross-encoder reranker（`_get_reranker` 已能在缺套件時優雅降級），和／或換更強的 embedder（現為 nomic-embed-text）。~~ | — | **不做** — ROI 不成立（見上）。重啟條件：`@ling` 問答開始給出明顯錯誤的答案。 |
...
| R7-C | ~~ChromaDB/FS 收斂 + adapter 邊界~~ ✅ 部分 2026-06-13 — **做了**：`_dereference_facets` 改 batched `_leading_chunks_for_docs`（N facets→1 `$in` get，消除每 facet 一次 round-trip + 重複）；`_format_part_digest_for_prompt`→公開 `format_digest_for_prompt`（移除 ingestion 跨邊界呼叫 LLM client 私有方法）。+1 facet 批次 test、rename 同步 4 test 檔。**延後（R7-C-2）**：insight_agent 8 處 `rag.collection.get(...)` 直存（純架構債，需 RagManager 加 ~3 公開方法 + rewire 8 site + 改 insight test mock，自成一批）；counter `rglob`（P7，僅 title-miss 冷路徑，O(vault) 但少觸發）；MMR re-embed（P9，cache 命中即近最佳）。 | 低–中 | 核心完成；架構債延後 R7-C-2 |
...
| R7-G | ~~**prompt_watcher dispatch-thread 阻塞**~~ ✅ 2026-06-13 — `_handle_event` 改為只 enqueue + 喚醒 worker 即返回（不再 sleep/drain on dispatch thread）；新增 daemon worker thread（`start()`/`stop()`，main.py 接生命週期）在自己的執行緒做穩定延遲 + `_drain_queue`。既有 busy-state（`try_set_busy`）與 `scan_existing` idle callback 互動不變、由 busy state 互斥序列化。`_stability_delay` 可注入（測試設 0）。+3 worker tests，910 passed。 | M | 完成 |
| R7-G-2 | ~~**clipping_watcher 同樣阻塞**~~ ✅ 2026-06-13 — 套用與 R7-G 相同的 worker-thread 修法：`_handle_event` 只 enqueue+喚醒，daemon worker 做穩定延遲 + `_drain_queue`；main.py 接 start()/stop()。+3 worker tests（新 test_clipping_watcher.py），913 passed。 | S–M | 完成 |
