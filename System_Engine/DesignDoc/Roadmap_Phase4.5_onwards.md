...
| R7-G-2 | **clipping_watcher 同樣阻塞**（R7-G 的姊妹）：`watchers/clipping_watcher.py` `_handle_event` 是與 prompt_watcher 一模一樣的 `sleep(1)+_drain_queue` on dispatch thread pattern。套同樣的 worker-thread 修法。 | S–M | 待排 |
