...
def _knob(metric, n, *, default=0.7, lo=0.3, hi=0.85, worse_when_high=True):
    return Tunable(
        ...
        target_hi=0.05,
        danger=0.10,
        worse_when_high=worse_when_high,
        is_int=False,
        min_samples=5,
    )
    ...
    save_state(
        {
            "params": {"K": 0.7},
            "changes": {},
            "last_tune": {"K": datetime(2026, 6, 13).isoformat()},
        },
        sf,
    )
    res = run_autotune(
        state_file=sf,
        now=datetime(2026, 6, 14),  # 1 day < 7
    ...
    save_state(
        {
            ...
            "last_tune": {"K": "2026-06-01T00:00:00"},
        },
        sf,
    )
    res = run_autotune(
        state_file=sf, now=datetime(2026, 6, 14), tunables=[_knob(metric=0.12, n=10)]
    )  # >= danger 0.10
    o = res.outcomes[0]
