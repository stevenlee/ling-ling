...
def _page(
    cortex_dir, claim, *, contradictions=(), evidence_insights=("i1.md",), status="active", **kw
):
    page = CortexPage(
        ...
        evidence=[
            {"insight": n, "sources": [], "date": "2026-06-01", "summary": "s"}
...
def _contradiction_triangle(cortex_dir, *, independent=True):
    ...
    _page(cortex_dir, b, contradictions=[ida], evidence_insights=("insight-one.md",))
    _page(
        cortex_dir,
        c,
        contradictions=[ida],
...
class TestFalsified:
    ...
    def test_one_contradiction_never_kills(self, tmp_path):
        ...
        _page(
            env["cortex_dir"], b, contradictions=[make_claim_id(a)], evidence_insights=("other.md",)
        )
        result = run_ledger_pass(FakeLLM(verdict="refuted"), FakeRAG(), **env)
    ...
    def test_refute_survival_blocks_kill_with_cooldown(self, tmp_path):
        env = _env(tmp_path)
        _contradiction_triangle(env["cortex_dir"], independent=True)
...
class TestUnmergeFeedback:
    def test_shrinkage_detected_and_strict_mode_engages(self, tmp_path):
        ...
        page = _page(
            env["cortex_dir"],
            "Merged page the user will split.",
        ...
        state["events"] = [{"kind": "merge", "claim_id": "x", "ts": "2026-06-10T00:00:00"}] * 4
...
class TestStrictModeInConsolidation:
    def test_equivalent_demotes_to_link_under_strict(self, tmp_path, monkeypatch):
        ...
        from tests.test_cortex_consolidation import (
            FakeLLM as ConsFakeLLM,
            FakeRAG as ConsFakeRAG,
            _write_insight,
        )
        ...
        llm1 = ConsFakeLLM(
            claims_map={"MARKER-A": [{"claim": "ALPHA base claim.", "summary": "s"}]}
        )
