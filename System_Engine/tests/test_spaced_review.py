...
def test_recalled_reinforces_and_persists(tmp_path):
    p = _make_page(tmp_path, "熵總是增加", days_ago=90)
    before = load_all_pages(tmp_path)[0]
    r_before = cortex_decay.retrievability(
        before.S, before.last_reinforced_at,
